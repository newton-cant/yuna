    ...
    try {
        ...
        Client.cmd.on('ytmp4', async (data) => {
            try {
                ...
                if(data.body == "") return data.reply(`coloca link depois do comando`)
                ...
                Client.sendFileFromUrl(data.from, `${ytm.link}`, `${ytm.title} - Download.mp4`, `O vídeo foi enviado @${data.sender.split('@')[0]}`, data.message)
            } catch {
                data.reply('error')
            }
        })
      ...
      Client.cmd.on('playvid', async (data) => {
            try {
                if(isLimit(data.sender)) return data.reply(mess.limit)
                if(data.body == "") return data.reply(`Qual nome do video?`)
                data.reply(mess.wait)
                res = await axios.get(`${configs.apiUrl}/api/ytplaymp4/2?apikey=${configs.zeksKey}&q=${data.body}`)
                if(res.data.status == false) data.reply(res.data.message)
                ytm = res.data.result
                teks = `*Dados obtidos com sucesso!*\n\n*Titulo* : ${ytm.title}\n*Tamanho* : ${ytm.size}\n*Qualidade* : ${ytm.quality}\n*Ext* : ${ytm.ext}\n*link* : ${ytm.source}\n\n_Aguarde o envio do arquivo de mídia; isso pode levar alguns minutos_`
                if(Number(ytm.size.split(' MB')[0]) >= 50.00) return Client.sendFileFromUrl(data.from, `${ytm.thumb}`, 'thumb.jpg', `*Dados obtido com sucesso!*\n\n*Titulo* : ${ytm.title}\n*Tamanho* : ${ytm.size}\n*Qualidade* : ${ytm.quality}\n*Ext* : mp4\n*Fonte* : ${ytm.source}\n*Link* : ${ytm.link}\n\n_Para a duração de mais do que o limite é apresentado na forma de um link_`, data.message)
                Client.sendFileFromUrl(data.from, `${ytm.thumb}`, 'thumb.jpg', teks, data.message)
                Client.sendFileFromUrl(data.from, `${ytm.link}`, 'video.mp4', `O vídeo foi enviado @${data.sender.split('@')[0]}`, data.message)
            } catch (e) {
                data.reply('Api invalida')
            }
        })
        Client.cmd.on('play', async (data) => {
            try {
                if(isLimit(data.sender)) return data.reply(`AGUARDE UM POUCO 💤`)
                ...
                ytm = res.data.result
                 teks = `*Dados obtidos com sucesso!*\n\n*Título* : ${ytm.title}\n*Tamanho* : ${ytm.size}\n*Qualidade* : ${ytm.quality}\n*Ext* : ${ytm.ext}\n*link* : ${ytm.source}\n\n_Aguarde o envio do arquivo de mídia; isso pode levar alguns minutos_`
           //"     if(Number(ytm..split(' MB')[0]) >= 50.00) return Client.sendFileFromUrl(data.from, `${ytm.thumb}`, 'thumb.jpg', `*Data Berhasil Didapatkan!*\n\n*Title* : ${ytm.title}\n*Ukuran* : ${ytm.size}\n*Kualitas* : ${ytm.quality}\n*Ext* : mp3\n*Source* : ${ytm.source}\n*Link* : ${ytm.link}\n\n_Untuk durasi lebih dari batas disajikan dalam bentuk link_`, data.message)
                Client.sendFileFromUrl(data.from, ytm.thumb, 'thumb.jpg', teks, data.message)
                Client.sendFileFromUrl(data.from, ytm.link, `${ytm.title} - Download.mp3`, ``, data.message)
            } catch {
                data.reply('error')
               ...
               Client.cmd.on('play2', async (data) => {
            try {
                if(isLimit(data.sender)) return data.reply(`AGUARDE UM POUCO 💤`)
                if(data.body == "") return data.reply(`Kirim perintah *${data.prefix}play2 [ link ]*\n: ${data.prefix}play2 alone`)
                data.reply(mess.wait)
                res = await axios.get(`https://server-api-rey.herokuapp.com/api/yt/playmp3?query=${data.body}&apikey=apirey`)
                if(res.data.status == false) data.reply(res.data.message)
                ytm = res.data
                 teks = `*Dados obtidos com sucesso!*\n\n*Título* : ${ytm.title}\n*Publicado* : ${ytm.published}\n*Views* : ${ytm.views}\n\n_Aguarde o envio do arquivo de mídia; isso pode levar alguns minutos_`
            if(Number(ytm.size.split(' MB')[0]) >= 50.00) return Client.sendFileFromUrl(data.from, `${ytm.thumb}`, 'thumb.jpg', `*Data Berhasil Didapatkan!*\n\n*Title* : ${ytm.title}\n*Ukuran* : ${ytm.size}\n*Kualitas* : ${ytm.quality}\n*Ext* : mp3\n*Source* : ${ytm.source}\n*Link* : ${ytm.link}\n\n_Untuk durasi lebih dari batas disajikan dalam bentuk link_`, data.message)
                Client.sendFileFromUrl(data.from, ytm.thumb, 'thumb.jpg', teks, data.message)
        ...
        Client.cmd.on('ig', async (data) => {
            if(isLimit(data.sender)) return data.reply(mess.limit)
            if(data.body == "") return data.reply(`coloque o link depois do comando`)
            data.reply(mess.wait)
            getresult = await axios.get(`${configs.apiUrl}/api/ig?apikey=${configs.zeksKey}&url=${data.body}`)
            if(getresult.data.status == false) return data.reply(`link inválido`)
        ...
        Client.cmd.on('igstory', async (data) => {
            try {
                if(isLimit(data.sender)) return data.reply(mess.limit)
                if(data.body == "") return data.reply(`Exemplo: ${data.prefix}igstory supra`)
                data.reply(mess.wait)
        ...
        Client.cmd.on('limit', async (data) => {
            ...
            if(dataUser[data.sender].premium) return data.reply(`Oi @${data.sender.split('@')[0]} 👋🏻\nVc é  membro premium não se preocupe com limite!`)
            limits = configs.maxLimit - dataUser[data.sender].limit
            if(limits <= 0) return data.reply("```" + `Limit anda sudah habis` + "```")
            data.reply(`Oi @${data.sender.split('@')[0]} 👋🏻\n Seu limite restante ${limits || 30}\nO limite é zerado todos os dias às 00h00\nSe você deseja obter um limite ilimitado,converse com meu criador !owner`)
        })
        ...
        Client.cmd.on('anime', async (data) => {
			try {
			if(isLimit(data.sender)) return data.reply(mess.limit)
            if(data.body == "") return data.reply(`Exemplo : ${data.prefix}anime naruto`)
            data.reply(mess.wait)
        ...
        Client.cmd.on('manga', async (data) => {
			try {
			if(isLimit(data.sender)) return data.reply(mess.limit)
            if(data.body == "") return data.reply(`Exemplo : ${data.prefix}manga naruto`)
            data.reply(mess.wait)
			...
			Client.sendFileFromUrl(data.from, image_url, 'p.jpg', `*Manga found!*\n\n*Title:* ${title}\n*Chapters:* ${chapters}\n*Rating:* ${rated}\n*Score:* ${score}\n*Synopsis:* ${synopsis}\n*URL*: ${url}`, data.message)
                ...
                data.reply('Mangá não encontrado')
            }
		})
        Client.cmd.on('chara', async (data) => {
			try {
			if(isLimit(data.sender)) return data.reply(mess.limit)
            if(data.body == "") return data.reply(`Exemplo : ${data.prefix}chara naruto`)
            data.reply(mess.wait)
        ...
        Client.cmd.on('block', async (data) => {
            if(!data.isOwner) return data.reply(mess.ownerOnly)
            if(data.mentionedJidList.length == 0) return data.reply(`Exemplo : ${data.prefix}${data.command} @0`)
            data.mentionedJidList.forEach(jids => client.blockUser(jids, "add"))
            data.reply(`bloqueado @${data.mentionedJidList.join(' @').replace(/@s.whatsapp.net/g, '')}`)
        })
        Client.cmd.on('unblock', async (data) => {
            if(!data.isOwner) return data.reply(mess.ownerOnly)
            if(data.mentionedJidList.length == 0) return data.reply(`Exemplo : ${data.prefix}${data.command} @0`)
            data.mentionedJidList.forEach(jids => client.blockUser(jids, "remove"))
            data.reply(`desbloqueado @${data.mentionedJidList.join(' @').replace(/@s.whatsapp.net/g, '')}`)
        })
        Client.cmd.on('addvn', async (data) => {
            if(!data.isOwner) return data.reply(mess.ownerOnly)
            if(!data.isQuotedAudio) return data.reply('Só vn/audio!')
            if(data.body == "") return data.reply(`Exemplo: ${data.command}addvn hai`)
            ...
            data.reply(`Adicionado com sucesso vn ${data.body} do banco de dados`)
        })
        Client.cmd.on('delvn', async (data) => {
            ...
            data.reply(`Vn excluída com sucesso ${data.body} do banco de dados`)
        })
        ...
        Client.cmd.on('tebakgambar', async (data) => {
			if(isLimit(data.sender)) return data.reply(mess.limit)
			if (global.tebakgambar[data.from] && global.tebakgambar[data.from].id) return data.reply("")
        ...
        Client.cmd.on('premium', async (data) => {
            ...
            if(data.args[0].toLowerCase() == 'add') {
                ...
                fs.writeFileSync('./lib/json/dataUser.json', JSON.stringify(dataUser))
                data.reply(`Usuário premium adicionado com sucesso @${dataToPr.join(' @').replace(/@s.whatsapp.net/g, '')}`)
                ...
                dataToPr.forEach(nums => {
                    ...
                    data.reply(`usuário premium excluído com sucesso @${nums.split('@')[0]}`)
                })
                fs.writeFileSync('./lib/json/dataUser.json', JSON.stringify(dataUser))
        ...
        Client.cmd.on('tempo', (data) => {
            ...
            Client.sendText(data.from, "```" + `${data.pushname} [@${data.sender.split('@')[0]}] \n\nRazão: ${data.body}\nTempo: ${timesNow}` + "```")
        })
	     Client.cmd.on('welcome', (data) => {
            ...
            if(data.args[0].toLowerCase() == 'on') {
                ...
                data.reply('Recurso de Bem-vindo ativo!')
                ...
                fs.writeFileSync('./lib/json/dataGc.json', JSON.stringify(dataGc))
                data.reply('Recurso de Bem-vindo desativado!')
            } else {
	    ...
	    Client.cmd.on('youtubedl', async (data) =>{
            if(isLimit(data.sender)) return data.reply(mess.limit)
            if(data.body == "") return data.reply(`Exemplo: ${data.prefix}youtubedl Alan walker`)
            data.reply(mess.wait)
			...
			let po = client.prepareMessageFromContent(data.from, {
				  "listMessage":{
                  "title": "*BAIXAR YOUTUBE*",
                  "description": `*Resultado : ${data.body}*\n*Baixe o vídeo clicando no botão*`,
                  "buttonText": "BOTÃO",
                  "listType": "SINGLE_SELECT",
                  "sections": secs}}, {}) 
            client.relayWAMessage(po, {waitForAck: true})	
        ...
        Client.cmd.on('revoke', (data) => {
            ...
            data.reply(`Link do grupo foi resetado com sucesso pelo administrador @${data.sender.split('@')[0]}`)
        })
        Client.cmd.on('group', (data) => {
            ...
            if(data.args[0] && data.args[0].toLowerCase() == 'open') {
                ...
                data.reply(`O grupo foi aberto pelo administrador @${data.sender.split('@')[0]}`)
...
`
        ...
        Client.cmd.on('kick', async (data) => {
            ...
            if(!data.botIsAdmin) return data.reply(mess.botAdmin)
            if(data.mentionedJidList.length == 0) return data.reply(`Exemplo : ${data.prefix}${data.command} @0`)
            data.mentionedJidList.forEach(async jid =>{ client.groupRemove(data.from, [jid]).then(x => data.reply(`Removido com sucesso @${jid.split('@')[0]}`)).catch(x => data.reply(`Falha ao chutar @${jid.split('@')[0]}`)); await sleep(2000)})
        })
        Client.cmd.on('add', async (data) => {
            ...
            if(!data.isAdmin) return data.reply(mess.admin)
            if(!data.botIsAdmin) return data.reply(mess.botAdmin)
            if(data.body == "") return data.reply(`Exemplo : ${data.prefix}${data.command} 55xxxx`)
            args = data.args.map(mp => mp + "@s.whatsapp.net")
            client.groupAdd(data.from, args).then(() => data.reply(`Adicionado com sucesso @${data.args.join(' @')}`)).catch(() => data.reply('Incapaz de convidar'))
        })
        ...
        Client.cmd.on('*', async (data) => {
            ...
            switch(command.toLowerCase()) {
				case 'self':
					...
					if (Client.self) return data.reply('Já no modo automático')
					Client.self = true
					data.reply('OK')
				break
				case 'public':
					if (!isOwner) return data.reply(mess.ownerOnly)
					if (!Client.self) return data.reply('Já no modo público')
					Client.self = false
					data.reply('OK')
				break
                case 'command':
                ...
                case 'stikergif':
                    if(isLimit(data.sender)) return data.reply(mess.limit)
                    if(type != 'videoMessage' && !isQuotedVideo && !isQuotedImage && type != 'imageMessage') return data.reply('marque uma imagem ou gif/video!')
                ...
                case 'tomp3':
                    if(isLimit(data.sender)) return data.reply(mess.limit)
					data.reply(mess.wait)
                    if(type != 'videoMessage' && !isQuotedVideo) return data.reply('marque um audio!')
                ...
                if (!isOwner) return data.reply(mess.ownerOnly)
                    if(isLimit(data.sender)) return data.reply(mess.limit)
                    if(type != 'videoMessage' && !isQuotedVideo && !isQuotedImage && type != 'imageMessage') return data.reply('marque uma imagem!')
                    if(data.body == "") return data.reply(`Exemplo: ${data.prefix}${data.command} supra|newton `)
                    data.reply(mess.wait)
                ...
                case 'semoji':
                    try {
                        if(isLimit(data.sender)) return data.reply(mess.limit)
                        if(data.body == "") return data.reply(`Exemplo : ${data.prefix}${data.command} 😃`)
           ...
           /*                    case 'stickeraa':
                    try {
                        if(isLimit(data.sender)) return data.reply(mess.limit)         
                        Client.sendStickerFromUrl(from, `${configs.apiUri}/api/stickera?apikey=supraz`)
                    } catch {
                        data.reply('error')
                    }
                    break*/
                ...
                case 'take':
                  if (!isOwner) return data.reply(mess.ownerOnly)
                    if(isLimit(data.sender)) return data.reply(mess.limit)
                    if(!data.isQuotedSticker) return data.reply('marque uma figurinha!')
                    if(data.body == "") return data.reply(`Exemplo : ${data.prefix}${data.command} Yuna|BOT`)
                    data.reply(mess.wait)
                    p = data.body
                    text = p.split('|')
            ...
            /*    case 'stickeraa':
                    ...
                    if(data.isQuotedImage || data.type == 'imageMessage') {
                        ...
                        ppUrl = await client.getProfilePicture(data.mentionedJidList[0])
                        if(!ppUrl) return data.reply('Profile picture not found!')
                        Client.sendStickerFromUrl(data.from, `https://supra-api.herokuapp.com/api/stickera?apikey=supraz`, data.message, {
                            pack: `${configs.pack}`,
                            author: `${configs.author}`
                        })
                    } else data.reply(`Wrong format!, tag someone or reply image with ${data.prefix}stickerfire`)
...
`
                ...
                case 'linkgroup':
                    ...
                    /*DLL*/
                case 'stickeraa':
        /*           porc = `${Math.floor(Math.random() * 51)}`
                    Client.sendRawWebpAsSticker(from, fs.readFileSync('./lib/stickerr/'+porc+'.webp'), message).then(resData => Client.sendText(from, 'gunakan sticker ini untuk menampilkan menu!', {
                        quoted: resData
                    })) */
                    Client.sendRawWebpAsSticker(from, await getBuffer('https://supra-api.herokuapp.com/api/stickera?apikey=supraz'), message).then(resData => Client.sendText(from, 'Figurinha aleatória', {
                        quoted: resData
                    }))
